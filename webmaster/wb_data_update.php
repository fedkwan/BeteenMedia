...
if ($_POST['data_password'] !== '')
	{
			...
			{echo "<script>alert('密码长度要大于6个字符，小于20个字符。'); location='wb_data.php';</script>";exit;}
...
if ($_POST['data_email'] !== '')
	{
			...
			{echo "<script>alert('请输入正确的Email'); location='wb_data.php';</script>";exit;}
		$query = "UPDATE user_form SET email = '$_POST[data_email]' WHERE username = '$_SESSION[uid]'";
		$result = mysql_query($query);
	}
echo "<script>alert('修改成功'); location='wb_data.php';</script>";exit;
}
else
{
echo "<script>alert('你未作任何修改'); location='wb_data.php';</script>";exit;
}
?>
</body>
</html>
